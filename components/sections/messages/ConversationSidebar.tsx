      ...
      <div className="bg-white dark:bg-[#121417] border border-zinc-200 dark:border-zinc-800/60 rounded-xl flex-1 flex flex-col overflow-hidden transition-colors duration-300">
        <div className="p-4 space-y-4 shrink-0 border-b border-zinc-100 dark:border-transparent transition-colors duration-300">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <span className="bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 text-[10px] px-1.5 py-0.5 rounded font-bold transition-colors duration-300">12 unread</span>
            </div>
          ...
          <div className="flex items-center justify-between text-[11px] font-bold transition-colors duration-300">
            <div className="flex space-x-4">
              <button className="text-zinc-900 dark:text-white border-b-2 border-zinc-900 dark:border-white pb-1 transition-colors duration-300">All</button>
              <button className="text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-300 pb-1 transition-colors duration-300">Unread</button>
              <button className="text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-300 pb-1 transition-colors duration-300">Pinned</button>
            </div>
            <span className="text-zinc-400 dark:text-zinc-600 uppercase text-[9px] tracking-widest transition-colors duration-300">Sort: Recent</span>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-zinc-100 dark:divide-zinc-800/30 scrollbar-hide transition-colors duration-300">
          {conversations.map((conv) => (
            <ConversationItem
              key={conv.id} name={conv.name} lastMessage={conv.lastMessage} type={conv.type}
              active={activeConv === conv.id} unread={conv.unread} onClick={() => setActiveConv(conv.id)}
            />
          ))}
        </div>
