...
export const VersionCard = ({ version: v, isLast, isLatest }: Props) => {
  ...
  const handleCompare = () => {
    ...
    router.push(`/resources/${resourceSlug}/history/compare?with=${v.collection_id}`);
  };
  ...
  return (
    <div className="relative group">
      ...
      <div className={`grid grid-cols-12 gap-6 border rounded-xl p-6 transition-all duration-300 ${
        isLatest 
        ...
        <div className="col-span-12 lg:col-span-8 space-y-4">
          <div className="flex items-center gap-3">
            <h4 className="text-sm font-bold text-zinc-900 dark:text-white uppercase tracking-tight">
              v{v.version_no} · {isLatest ? "Current Version" : "Archive Snapshot"}
            </h4>
          ...
          <div className="flex items-center gap-4 text-[11px] text-zinc-500 font-medium">
            <span className="flex items-center gap-1.5"><User size={12}/> {v.author}</span>
            <span className="flex items-center gap-1.5"><Clock size={12}/> {dateFormatted}</span>
            <span className="flex items-center gap-1.5"><FileText size={12}/> {v.file_count} files</span>
          </div>
        </div>
        ...
        <div className="col-span-12 lg:col-span-4 flex flex-col justify-center gap-2">
          ...
          {!isLatest && (
            <button className="w-full bg-emerald-100 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/20 py-2 rounded font-bold text-[11px] hover:bg-emerald-200 transition-all">
