...
const ContentTypeCard: React.FC<ContentTypeCardProps> = ({ icon: Icon, title, description, active, onClick }) => (
  <div 
    ...
    className={`flex-1 min-w-[280px] p-5 rounded-xl border cursor-pointer transition-all duration-300 ${
      active 
        ? 'bg-[#1a1d21] border-blue-500/50 ring-1 ring-blue-500/20' 
        : 'bg-[#121417] border-zinc-800/60 hover:border-zinc-700'
    }`}
  >
    <div className="flex items-start gap-4">
      <div className={`p-3 rounded-lg ${active ? 'bg-blue-500/10 text-blue-400' : 'bg-zinc-900 text-zinc-500'}`}>
        <Icon size={20} />
      </div>
      <div>
        <h3 className="text-white font-bold text-[14px]">{title}</h3>
        <p className="text-zinc-500 text-[12px] mt-1 leading-relaxed">{description}</p>
      </div>
    </div>
  </div>
);
...
const GeneratedCard: React.FC<GeneratedCardProps> = ({ title, subject, description, tags, type }) => (
  <div className="bg-[#121417] border border-zinc-800/60 rounded-xl p-5 hover:border-zinc-700 transition-all group">
    <div className="flex justify-between items-start mb-4">
      <div>
        <h4 className="text-white font-bold text-[15px] group-hover:text-emerald-400 transition-colors">{title}</h4>
        <p className="text-zinc-500 text-[12px] mt-1">{subject}</p>
      </div>
    </div>
    
    <div className="mb-4">
      <h5 className="text-zinc-400 text-[11px] font-bold uppercase tracking-wider mb-2">{type}</h5>
    ...
    <div className="flex flex-wrap gap-2 mb-6">
      {tags.map((tag, i) => (
        <span key={i} className="px-2.5 py-1 rounded bg-zinc-900 border border-zinc-800 text-[10px] text-zinc-500 font-medium">
          {tag}
        </span>
      ))}
    </div>
    ...
    <div className="flex gap-2 mt-auto">
      <button className="flex-1 flex items-center justify-center gap-2 bg-blue-600/10 hover:bg-blue-600/20 text-blue-400 py-2 rounded-lg text-[12px] font-bold transition-all border border-blue-500/20">
        <Download size={14} />
        Download
      </button>
      <button className="flex items-center justify-center p-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded-lg transition-all border border-red-500/20">
        <X size={16} />
      </button>
    </div>
  </div>
        ...
        <div className="p-8 max-w-6xl mx-auto w-full space-y-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="max-w-xl">
              ...
              <p className="text-zinc-500 text-[14px] mt-2 leading-relaxed">
                Generate customized lesson plans, teaching strategies, and classroom materials powered by AI.
              </p>
            </div>
            <div className="flex bg-zinc-900/40 p-1 rounded-xl border border-zinc-800/60 self-start">
              <button 
                onClick={() => setActiveView('generator')}
                className={`px-5 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-2 ${activeView === 'generator' ? 'bg-emerald-500 text-zinc-950 shadow-lg shadow-emerald-500/10' : 'text-zinc-500 hover:text-zinc-300'}`}
              >
                <Wand2 size={14} />
                Generate Content
              </button>
              <button 
                ...
                className={`px-5 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-2 ${activeView === 'results' ? 'bg-emerald-500 text-zinc-950 shadow-lg shadow-emerald-500/10' : 'text-zinc-500 hover:text-zinc-300'}`}
              >
                <Sparkles size={14} />
                My Generated (2)
              </button>
          ...
          {activeView === 'generator' ? (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
              ...
              {/* Input Form */}
              <div className="bg-[#121417] border border-zinc-800/60 rounded-2xl p-8 space-y-8 relative overflow-hidden">
                <div className="relative z-10">
                  <h2 className="text-lg font-bold text-white">Content Details</h2>
                  <p className="text-zinc-500 text-xs mt-1">Provide information to customize your generated content.</p>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        <select 
                            ...
                            className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-4 py-2.5 text-[13px] appearance-none focus:outline-none focus:ring-1 focus:ring-emerald-500/30 text-zinc-300"
                        >
                  ...
                  <div className="mt-8 space-y-2">
                    ...
                    <textarea 
                      ...
                      rows={5}
                      className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-5 py-4 text-[13px] focus:outline-none focus:ring-1 focus:ring-emerald-500/30 text-zinc-300 resize-none placeholder:text-zinc-700 leading-relaxed"
                    />
                  </div>
                  ...
                  <div className="mt-8 space-y-4">
                    <button 
                      ...
                      className={`w-full py-3.5 rounded-xl font-bold text-[14px] transition-all flex items-center justify-center gap-3 ${
                        ...
                        : 'bg-emerald-500 hover:bg-emerald-400 text-zinc-950 active:scale-[0.99] shadow-lg shadow-emerald-500/5'
                      }`}
                    >
                    ...
                    {isGenerating && (
                      <div className="space-y-3 animate-in fade-in">
                        ...
                        <p className="text-center text-zinc-500 text-[11px] font-medium italic">
                    ...
                    {error && (
                        <div className="flex items-center gap-2 text-red-500 text-[12px] font-bold bg-red-500/10 p-3 rounded-lg border border-red-500/20 animate-in shake-in duration-200">
                            <AlertCircle size={14} />
                            {error}
                        </div>
                    )}
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
              <div className="flex items-center justify-between">
                <div>
                   <h2 className="text-xl font-bold text-white">Your Generated Content</h2>
                   <p className="text-zinc-500 text-[13px] mt-1">View and manage all AI-generated lesson plans, strategies, and classroom materials.</p>
                </div>
                <button 
                  onClick={() => setActiveView('generator')}
                  className="bg-blue-600/10 hover:bg-blue-600/20 text-blue-400 px-5 py-2.5 rounded-xl border border-blue-500/20 text-[13px] font-bold flex items-center gap-2 transition-all"
                >
                  <Wand2 size={16} />
                  Generate More
                </button>
              ...
              <div className="bg-zinc-900/30 p-8 rounded-2xl border border-zinc-800/60 border-dashed text-center">
                 ...
                 <h4 className="text-zinc-400 font-bold">Need something else?</h4>
                 <p className="text-zinc-600 text-sm mt-1 max-w-sm mx-auto">Use the AI Generator to create more custom resources for your classes in seconds.</p>
              </div>
            </div>
          )}
        </div>
